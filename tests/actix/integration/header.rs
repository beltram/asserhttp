...
mod eq {
    ...
    async fn should_expect_header() {
        ...
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header("x-a", "a");
    }
    ...
    async fn expect_header_should_panic_when_wrong_key() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header("x-b", "a");
    }
    ...
    async fn expect_header_should_panic_when_wrong_value() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header("x-a", "b");
    }
    ...
    async fn expect_header_should_panic_when_multi_valued() {
        ...
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header("x-m", "a");
    }
}
...
mod multi {
    ...
    async fn expect_headers_should_expect_multi_headers() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-m", ["a", "b"]);
    }
    ...
    async fn expect_headers_should_fail_when_key_missing() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-b", ["a", "b"]);
    }
    ...
    async fn expect_headers_should_fail_when_one_value_missing() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-m", ["a"]);
    }
    ...
    async fn expect_headers_should_fail_when_one_value_not_eq() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-m", ["a", "c"]);
    }
    ...
    async fn expect_headers_should_fail_when_one_expected_value_missing() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-m", ["a", "b", "c"]);
    }
    ...
    async fn expect_headers_should_fail_when_no_value_expected() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-m", "a, b")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_headers("x-m", []);
    }
}
...
mod present {
    ...
    async fn should_expect_header_present() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header_present("x-a");
    }
    ...
    async fn expect_header_present_should_fail_when_absent() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header_present("x-b");
    }
}
...
mod absent {
    ...
    async fn should_expect_header_absent() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header_absent("x-b");
    }
    ...
    async fn expect_header_absent_should_fail_when_absent() {
        let app = App::new()
            .route("/", web::get().to(|| async { HttpResponse::Ok().append_header(("x-a", "a")).finish() }));
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_header_absent("x-a");
    }
...
mod json {
    ...
    async fn should_expect_content_type_json() {
        ...
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_content_type_json();
    }
...
mod text {
    ...
    async fn should_expect_content_type_text() {
        ...
        call_service(&init_service(app).await, TestRequest::get().to_request()).await.expect_content_type_text();
    }
