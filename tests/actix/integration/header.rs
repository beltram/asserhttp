...
mod eq {
    ...
    async fn should_expect_header() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("x-a", "a").finish() }));
    ...
    async fn should_expect_many_header() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("x-a", "a").header("x-b", "b").finish() }));
    ...
    async fn expect_header_should_panic_when_multi_valued() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("x-m", "a, b").finish() }));
...
mod json {
    ...
    async fn should_expect_content_type_json() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("content-type", "application/json").finish() }));
    ...
    #[actix_rt::test]
    async fn expect_header_should_panic_when_wrong_value() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("content-type", "application/xml").finish() }));
        call_service(&mut init_service(app).await, TestRequest::get().to_request()).await.expect_content_type_json();
    }
}
...
mod text {
    ...
    async fn should_expect_content_type_text() {
        let app = App::new().route("/", web::get().to(|| async { HttpResponse::Ok().header("content-type", "text/plain").finish() }));
