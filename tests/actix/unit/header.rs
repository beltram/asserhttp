...
mod eq {
    ...
    async fn should_expect_header() {
        async fn one(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-a", "a").finish() }
    ...
    async fn should_expect_many_header() {
        async fn many(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-a", "a").header("x-b", "b").finish() }
    ...
    async fn expect_header_should_panic_when_multi_valued() {
        async fn multi(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-m", "a, b").finish() }
        multi(TestRequest::get().to_http_request()).await.expect_header("x-m", "a");
    }

    #[actix_rt::test]
    async fn result_should_expect_header() {
        fn one(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-a", "a").finish() }
        one(TestRequest::get().to_http_request()).await.expect_header("x-a", "a");
    }

    #[actix_rt::test]
    async fn result_should_expect_many_header() {
        fn many(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-a", "a").header("x-b", "b").finish() }
        many(TestRequest::get().to_http_request()).await
            .expect_header("x-a", "a")
            .expect_header("x-b", "b");
    }
    ...
    async fn result_expect_header_should_panic_when_multi_valued() {
        fn multi(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-m", "a, b").finish() }
        multi(TestRequest::get().to_http_request()).await.expect_header("x-m", "a");
    }
}
...
mod absent {
    ...
    async fn should_expect_header_absent() {
        async fn ok(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("x-a", "a").finish() }
...
mod json {
    ...
    async fn should_expect_content_type_json() {
        async fn json(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "application/json").finish() }
    ...
    #[actix_rt::test]
    async fn expect_header_should_panic_when_wrong_value() {
        async fn xml(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "application/xml").finish() }
        xml(TestRequest::get().to_http_request()).await.expect_content_type_json();
    }

    #[actix_rt::test]
    async fn result_should_expect_content_type_json() {
        fn json(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "application/json").finish() }
        json(TestRequest::get().to_http_request()).await.expect_content_type_json();
    }
    ...
    #[actix_rt::test]
    async fn result_expect_header_should_panic_when_wrong_value() {
        fn xml(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "application/xml").finish() }
        xml(TestRequest::get().to_http_request()).await.expect_content_type_json();
    }
}
...
mod text {
    ...
    async fn should_expect_content_type_text() {
        async fn text(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "text/plain").finish() }
    ...
    async fn result_should_expect_content_type_text() {
        fn text(_: HttpRequest) -> HttpResponse { HttpResponse::Ok().header("content-type", "text/plain").finish() }
        text(TestRequest::get().to_http_request()).await.expect_content_type_text();
    }
